...
Sg.theme("DarkBlue")
layout_x = []
con_b = [[Sg.Button('Console', size=(10, 1), button_color=('white', 'green'), key='-Console-')]]
for j in range(len(links)):
    layout_x += [[Sg.Text(just_name[j], font='any 15')],
                 [Sg.Button('Download PART1', size=(15, 1), key=f'Download{j}'),
                  Sg.Button('Download PART2', size=(15, 1), key=f'Download-node{j}'),
                  Sg.Text("              "), Sg.Button('Launch \'' + just_name[j] + '\'', size=(15, 1),
                                                       key=f'Launch{j}'),
...
layout = [
    [Sg.Column(con_b, justification='r')],
    [Sg.Column(layout_x)]]

window = Sg.Window('Launcher', layout, icon=img_data, use_default_focus=False)
...
while True:
    ...
    if event == Sg.WINDOW_CLOSED:
        ...
        Console_SWITCH = not Console_SWITCH
        window.Element('-Console-').Update(('No Console', 'Console')[Console_SWITCH],
                                           button_color=('white', ('gray', 'green')[Console_SWITCH]))
    for j in range(len(links)):
        ...
        if event == f'Download-node{j}':
            ...
            if Console_SWITCH:
                def start_process():
                    call(f_name, start_new_session=True)
                thread_start_process = threading.Thread(target=start_process)
                thread_start_process.start()
            if not Console_SWITCH:
                Popen(f_name, shell=True, start_new_session=True)
