...
auth = False
if not os.path.isdir('./Essentials/' + time_str):
    if not os.path.isdir('./Essentials/'):
        os.mkdir('./Essentials/')
    os.mkdir('./Essentials/' + time_str)

acc_f_path = './Essentials/' + time_str + '/AccCount.txt'
vi_f_path = './Essentials/' + time_str + '/UsedIPs.txt'
...
if auth:
    ...
    at_time = diff if (diff < 10) else 10
    ...
    def main_menu():
        ...
        def start_function():
            ...
            if m_vpn_switch:
                ...
                vpn_ip_csv = pd.read_csv(StringIO(vpn_ip_response.text), usecols=[vpn_name.strip()]).T.values.tolist()[0]
        ...
        def start_process():
            ...
            if Vpn_Connected:
                ...
                try:
                    ...
                    call('taskkill.exe /F /IM chrome.exe', shell=True)
                CurrIP = requests.get('https://ipinfo.io/ip?token='+random.choice(tk)).text
                open(vi_f_path, "a").write(CurrIP + '\n')
                call('taskkill.exe /F /IM openvpn.exe', shell=True)
                Popen(add_route, shell=True, start_new_session=True, stdin=PIPE, stdout=PIPE, stderr=PIPE)
                if int(open(acc_f_path, 'r').readline().strip()) + 1 <= Acc_Create:
                    Vpn_Connected = False
        ...
        login_button = [[Sg.Button("Save", size=(10, 1), visible=True)]]
        kill_switch_button = [[Sg.Button('K/S ON', size=(10, 1), button_color=('white', 'green'), key='Kill-Switch')]]
        login_layout = [[Sg.Text("Log In", justification='center', font='Any 25')],
                        ...
                        [Sg.Column(login_button, element_justification='c')]]
        tab0_layout = [[Sg.Text('Status: ', key='Connect-Status'), Sg.Text('IP :', key='Current-IP'),
                       ...
                       [Sg.T('1', key='_RIGHT1_'),
                        Sg.Slider((1, at_time), default_value=1, key='_SLIDER_AAT_', orientation='h', enable_events=True,
                                  size=(5, 0.4)), Sg.T(at_time)], [Sg.Text("Select How many videos you want to play")],
        ...
        tab_group_layout = [
            [Sg.Tab('                                        Menu                                       ',
                    tab0_layout, font='Courier 15', key='-TAB1-'),
             Sg.Tab('                                        Login                                      ',
                    login_layout, visible=True, key='-TAB2-')]]

        layout = [[Sg.TabGroup(tab_group_layout, title_color='#0f0f0f', enable_events=True, key='-TAB0-')],
        ...
        while True:
            ...
            if event is None or event == 'Exit':
                ...
                username_login = values['-usrnm-']
                password_login = values['-pwd-']
                open('./Essentials/myauthfile.txt', 'w').write(username_login + "\n")
                open('./Essentials/myauthfile.txt', 'a').write(password_login + "\n")
